...
def evaluate_individual(individual, data):
    return formulas.fitness(individual, data)

def evaluate_population(population, data):
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        scores = pool.starmap(evaluate_individual, [(individual, data) for individual in population])
    return scores

def next_generation(population, data, mutation_rate):
    # Score each individual in the population using the fitness function
    population_scores = evaluate_population(population, data)
    population_score = dict(zip(population, population_scores))
